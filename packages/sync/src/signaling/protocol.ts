 ...
 * byte (writeVarUint).
 *
 * NOTE: This is a copy of the protocol defined in
 * @pokapali/node. Both packages encode/decode the
 * same wire format. The duplication exists because
 * @pokapali/sync cannot depend on @pokapali/node
 * (Node-only) and @pokapali/core already depends
 * on @pokapali/sync (circular). Property tests in
 * both packages verify encoding compatibility.
 *
 * @module
 */
