...
async function makeTestKeys() {
  const keys = await deriveDocKeys(
    "test-secret", "test-app", ["doc"]
  );
  const seed = keys.ipnsKeyBytes;
  const signingKey =
    await ed25519KeyPairFromSeed(seed);
  return { readKey: keys.readKey, signingKey };
}

async function cidFromBytes(
  bytes: Uint8Array
): Promise<CID> {
...
describe("@pokapali/snapshot", () => {
  describe("encodeSnapshot / decodeSnapshot",
    () => {
      it("round-trips CBOR encoding", async () => {
        ...
        expect(node.publicKey).toEqual(
          signingKey.publicKey
        );
        expect(node.signature)
          .toBeInstanceOf(Uint8Array);
        expect(node.subdocs.doc)
          .toBeInstanceOf(Uint8Array);
      });
    }
  );
  ...
  describe("decryptSnapshot", () => {
    it("decrypts subdoc payloads", async () => {
      ...
      expect(decrypted.doc).toEqual(
        plaintext.doc
      );
      expect(decrypted.meta).toEqual(
        plaintext.meta
      );
    });
  });
  ...
  describe("validateStructure", () => {
    it("returns true for valid snapshot",
      async () => {
        ...
        expect(
          await validateStructure(encoded)
        ).toBe(true);
      }
    );
    ...
    it("returns false for tampered data",
      async () => {
        ...
        expect(
          await validateStructure(tampered)
        ).toBe(false);
      }
    );
  ...
  describe("walkChain", () => {
    it("walks from tip to genesis",
      async () => {
        ...
        const genesisCid =
          await cidFromBytes(genesis);
        blocks.set(
          genesisCid.toString(), genesis
        );

        // Second block
        const second = await encodeSnapshot(
          { doc: new Uint8Array([2]) },
          readKey, genesisCid, 1, 2000,
          signingKey
        );
        const secondCid =
          await cidFromBytes(second);
        blocks.set(
          secondCid.toString(), second
        );

        // Third block
        const third = await encodeSnapshot(
          { doc: new Uint8Array([3]) },
          readKey, secondCid, 2, 3000,
          signingKey
        );
        const thirdCid =
          await cidFromBytes(third);
        blocks.set(
          thirdCid.toString(), third
        );
        ...
        for await (
          const node of walkChain(
            thirdCid, getter
          )
        ) {
          nodes.push({
            seq: node.seq, ts: node.ts
          });
        }
    ...
    it("yields single node for genesis",
      async () => {
        const { readKey, signingKey } =
          await makeTestKeys();
        const genesis = await encodeSnapshot(
        ...
        const nodes: number[] = [];
        for await (
          const node of walkChain(cid, getter)
        ) {
          nodes.push(node.seq);
        }
  ...
  describe("FetchCoalescerState", () => {
    ...
    it("coalescerNext moves pending to " +
      "inflight", () => {
      const s = createFetchCoalescerState();
      s.pending.add("cidA");
      s.pending.add("cidB");
    ...
    it("coalescerResolve moves inflight to " +
      "resolved", () => {
      const s = createFetchCoalescerState();
      s.inflight.add("cidA");
      ...
      expect(s.inflight.has("cidA"))
        .toBe(false);
      expect(s.resolved.get("cidA"))
        .toEqual(block);
    });

    it("coalescerFail moves inflight to " +
      "failed", () => {
      const s = createFetchCoalescerState();
      s.inflight.add("cidA");
