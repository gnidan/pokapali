...
async function enqueuePublish(keyHex, cid, seq, doPublish) {
    ...
    const run = async (currentCid, currentSeq) => {
        try {
 ...
 */
export async function publishIPNS(helia, ipnsKeyBytes, cid, clockSum) {
    const keyHex = Array.from(ipnsKeyBytes, (b) => b.toString(16).padStart(2, "0")).join("");
    await enqueuePublish(keyHex, cid, BigInt(clockSum), async (cidToPublish, seq) => {
        log.debug("doPublish: seq=" + seq + " cid=" +
            cidToPublish.toString().slice(0, 16) +
            "...");
        const privateKey = await generateKeyPairFromSeed("Ed25519", ipnsKeyBytes);
        const delegated = helia.libp2p.services
            .delegatedRouting;
        if (!delegated?.putIPNS) {
            log.warn("no delegatedRouting.putIPNS"
                + " — skipping");
            return;
        }
        const keyCid = CIDClass.createV1(LIBP2P_KEY_CODEC, privateKey.publicKey.toMultihash());
        ...
        if (delegated.getIPNS) {
            try {
                const existing = await delegated.getIPNS(keyCid, {
                    signal: AbortSignal.timeout(5_000),
                });
                const existingSeq = existing.sequence ?? 0n;
                log.debug("existing seq=" + existingSeq +
                    " clockSum seq=" + seq);
        ...
        log.debug("publishing with effectiveSeq=" +
            effectiveSeq);
        const record = await createIPNSRecord(privateKey, cidToPublish, effectiveSeq, DEFAULT_LIFETIME_MS);
...
export async function resolveIPNS(helia, publicKeyBytes) {
    ...
    if (delegated?.getIPNS) {
        ...
        try {
            const record = await delegated.getIPNS(keyCid, {
            ...
            log.debug("delegated resolve failed,"
                + " falling back to DHT:", err);
        }
    }
    else {
...
export function watchIPNS(helia, publicKeyBytes, onUpdate, intervalOrOpts) {
...
//# sourceMappingURL=ipns-helpers.js.map