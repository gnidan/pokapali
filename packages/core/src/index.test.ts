...
describe("@pokapali/core", () => {
  ...
  it(
    "create() subdoc(ns) returns Y.Doc",
    async () => {
      const lib = createCollabLib(OPTS);
      const doc = await lib.create();
  ...
  it(
    "create() subdoc(unknown) throws",
    async () => {
      const lib = createCollabLib(OPTS);
      const doc = await lib.create();
      expect(() => doc.subdoc("nonexistent"))
        .toThrow(
          /Unknown namespace "nonexistent"/
        );
      expect(() => doc.subdoc("nonexistent"))
        .toThrow(/content, comments/);
      doc.destroy();
    }
  );

  it(
    "create() capability is admin",
    async () => {
      const lib = createCollabLib(OPTS);
      const doc = await lib.create();
      expect(doc.capability.isAdmin).toBe(true);
      expect(doc.capability.canPushSnapshots)
        .toBe(true);
      expect(doc.capability.namespaces)
        .toEqual(
  ...
  it(
    "create() provider.awareness exists",
    async () => {
      const lib = createCollabLib(OPTS);
      const doc = await lib.create();
      expect(doc.provider.awareness)
        .toBeDefined();
      expect(doc.awareness)
        .toBe(doc.provider.awareness);
      doc.destroy();
    }
  );

  it(
    "create() URLs are pre-computed strings",
    async () => {
      const lib = createCollabLib(OPTS);
      const doc = await lib.create();
      expect(doc.adminUrl).toBeTypeOf("string");
  ...
  it(
    "open(url) infers reader capability",
    async () => {
      const lib = createCollabLib(OPTS);
      ...
      expect(reader.capability.isAdmin)
        .toBe(false);
      expect(
        reader.capability.canPushSnapshots
      ).toBe(false);
      expect(
        reader.capability.namespaces.size
      ).toBe(0);
  ...
  it(
    "pushSnapshot() no-op for readers",
    async () => {
      const spy = vi.mocked(encodeSnapshot);
      const lib = createCollabLib(OPTS);
      const admin = await lib.create();
  ...
  it(
    "inviteUrl returns narrowed URL",
    async () => {
      const lib = createCollabLib(OPTS);
      const doc = await lib.create();
  ...
  it(
    "inviteUrl throws on escalation",
    async () => {
      const lib = createCollabLib(OPTS);
      const admin = await lib.create();
      const readUrl = admin.readUrl;
  ...
  it(
    ...
    async () => {
      ...
      expect(doc.status)
        .toBe("unpushed-changes");
      doc.destroy();
    }
  );

  it(
    "on('status') fires on change",
    async () => {
      const lib = createCollabLib(OPTS);
      const doc = await lib.create();
  ...
  it(
    ...
    async () => {
      ...
      const doc = await lib.create();
      doc.destroy();
      expect(() => doc.subdoc("content"))
        .toThrow(/destroyed/);
    }
  );

  it(
    "_meta populated on create",
    async () => {
      const lib = createCollabLib(OPTS);
      const doc = await lib.create();
