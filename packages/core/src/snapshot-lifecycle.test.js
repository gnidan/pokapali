...
describe("createSnapshotLifecycle", () => {
    ...
    it("applyRemote updates seq/prev when remote " +
        "is newer", async () => {
        const readKey = await crypto.subtle.generateKey({ name: "AES-GCM", length: 256 }, true, ["encrypt", "decrypt"]);
        const lc = createSnapshotLifecycle({
            getHelia: () => mockHelia,
        ...
        const block = await realEncodeSnapshot(plaintext, readKey, null, 5, Date.now(), remoteSigningKey);
        const hash = await sha256.digest(block);
        const cid = CID.createV1(0x71, hash);
        vi.mocked(fetchBlock).mockResolvedValue(block);
        const applied = [];
        const result = await lc.applyRemote(cid, readKey, (snap) => { applied.push(snap); });
...
//# sourceMappingURL=snapshot-lifecycle.test.js.map