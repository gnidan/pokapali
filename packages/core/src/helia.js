...
let sharedHelia = null;
let refCount = 0;
export async function acquireHelia(options) {
    if (sharedHelia) {
        refCount++;
        return sharedHelia;
    }
    const isSecureContext = typeof globalThis.location !== "undefined" &&
    ...
    const helia = await Promise.race([
        ...
        new Promise((_, reject) => setTimeout(() => reject(new Error("Helia bootstrap timed out")), BOOTSTRAP_TIMEOUT_MS)),
    ]);
    sharedHelia = helia;
    refCount = 1;
    return helia;
}
export async function releaseHelia() {
...
export function getHeliaPubsub() {
    if (!sharedHelia) {
 ...
 */
export function _resetHeliaState() {
    sharedHelia = null;
    refCount = 0;
}
