 ...
 */
export function startRoomDiscovery(helia, _appId) {
    let stopped = false;
    let cycleController = null;
    const relayPeerIds = new Set();
    const relayAddrs = new Map();
    let running = false;
    const secure = typeof globalThis.location !== "undefined" &&
        globalThis.location.protocol === "https:";
    function wssAddrs(pid, rawAddrs) {
        return extractWssAddrs(pid, rawAddrs, secure);
    }
    function tagRelay(pid) {
        const conn = helia.libp2p.getConnections()
            .find((c) => c.remotePeer.toString() === pid);
        if (!conn)
            return;
        helia.libp2p.peerStore.merge(conn.remotePeer, {
            tags: {
                [RELAY_TAG]: { value: RELAY_TAG_VALUE },
            },
        }).catch(() => { });
    }
    function untagRelay(pid) {
        const conn = helia.libp2p.getConnections()
            .find((c) => c.remotePeer.toString() === pid);
        if (!conn)
            return;
        helia.libp2p.peerStore.merge(conn.remotePeer, { tags: { [RELAY_TAG]: undefined } }).catch(() => { });
    }
    function trackRelay(pid, addrs) {
        relayPeerIds.add(pid);
        relayAddrs.set(pid, addrs);
        tagRelay(pid);
    }
    function untrackRelay(pid) {
        relayPeerIds.delete(pid);
        relayAddrs.delete(pid);
        untagRelay(pid);
    }
    async function dialRelay(pid, wssAddrs, rawAddrs, peerId) {
        const short = pid.slice(-8);
        log.debug(`relay ...${short}, dialing...`, wssAddrs.map((ma) => ma.toString()));
    ...
    async function discoverRelays() {
        if (running)
            return; // skip if previous cycle active
        running = true;
        ...
        try {
            ...
            const toDial = [];
            let found = 0;
            for await (const provider of helia.routing.findProviders(cid, { signal })) {
                if (signal.aborted)
                    break;
                found++;
                ...
                const dialable = addrs.some((a) => a.includes("/ws/") ||
                    a.includes("/ws ") ||
                    a.endsWith("/ws") ||
                    a.includes("/wss/") ||
                    a.endsWith("/wss") ||
            ...
            if (toDial.length > 0) {
                await Promise.allSettled(toDial.map(async (p) => {
    ...
    function logStatus() {
        ...
        log.debug(`${peers.length} peers,`, `${addrs.length} listening addrs`);
    }
    ...
    const disconnectHandler = (evt) => {
        ...
        if (attempts >= MAX_RECONNECT_ATTEMPTS) {
            log.info(`relay ...${short} disconnected,`, `max attempts reached — removing`);
        ...
        const delay = RECONNECT_BASE_DELAY_MS
            * Math.pow(2, attempts);
        log.info(`relay ...${short} disconnected,`, `redial in ${delay / 1000}s`, `(attempt ${attempts + 1})`);
        reconnectTimers.set(pid, setTimeout(async () => {
    ...
    async function addExternalRelays(entries) {
        for (const entry of entries) {
            if (stopped)
                break;
            if (relayPeerIds.has(entry.peerId))
                continue;
            const pid = entry.peerId;
            const short = pid.slice(-8);
            const already = helia.libp2p
    ...
    return {
        ...
        relayEntries() {
            return [...relayAddrs.entries()].map(([peerId, addrs]) => ({ peerId, addrs }));
        },
