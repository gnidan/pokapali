...
// --- mocks ---
const { mockCreateIPNSRecord, mockGenerateKeyPairFromSeed, mockPublicKeyFromRaw, mockIpns, mockResolve, } = vi.hoisted(() => {
    const mockCreateIPNSRecord = vi.fn().mockResolvedValue({ value: "/ipfs/test" });
...
describe("publishIPNS", () => {
    ...
    it("creates record with clock sum seq and publishes via delegated routing", async () => {
        ...
        expect(mockCreateIPNSRecord).toHaveBeenCalledWith(fakePrivKey, cid, 42n, expect.any(Number));
        // delegated HTTP publish
        expect(mockPutIPNS).toHaveBeenCalledWith(expect.anything(), fakeRecord, expect.objectContaining({
    ...
    it("uses existing seq + 1 when clock sum is lower", async () => {
        ...
        expect(mockCreateIPNSRecord).toHaveBeenCalledWith(fakePrivKey, cid, 101n, expect.any(Number));
    });
});
...
//# sourceMappingURL=ipns-helpers.test.js.map