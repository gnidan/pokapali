...
describe("startRoomDiscovery", () => {
    ...
    describe("stop() cleanup", () => {
        it("clears intervals (no more discovery " +
            ...
            const rd = startRoomDiscovery(helia);
            // Let initial async work settle
            await vi.advanceTimersByTimeAsync(1);
            const callsBefore = helia.routing.findProviders.mock.calls
                .length;
            rd.stop();
            // Advance past several discovery intervals
            await vi.advanceTimersByTimeAsync(120_000);
            // No new findProviders calls after stop
            expect(helia.routing.findProviders.mock.calls
                .length).toBe(callsBefore);
        });
        it("removes event listeners", () => {
            const rd = startRoomDiscovery(helia);
            // Should have registered disconnect listener
            expect(helia.libp2p.addEventListener).toHaveBeenCalledWith("peer:disconnect", expect.any(Function));
            rd.stop();
            // Listener should be removed
            expect(helia.libp2p.removeEventListener).toHaveBeenCalledWith("peer:disconnect", expect.any(Function));
            // Internal listener set should be empty
            expect(helia._listeners.get("peer:disconnect")
                ?.size ?? 0).toBe(0);
        });
        it("aborts active discovery cycle", async () => {
            ...
            helia.routing.findProviders =
                vi.fn(async function* (_cid, opts) {
    ...
    describe("disconnect handler", () => {
        ...
        async function setupTrackedRelay(h) {
            const rd = startRoomDiscovery(h);
            await vi.advanceTimersByTimeAsync(1);
        ...
        it("keeps relay tracked during reconnect " +
            "and triggers redial after backoff", async () => {
            vi.mocked(loadCachedRelays)
                .mockReturnValue([{
                    peerId: RELAY_PID,
                    addrs: RELAY_ADDRS,
            ...
            expect(rd.relayPeerIds.has(RELAY_PID))
                .toBe(true);
            expect(rd.relayEntries().some((e) => e.peerId === RELAY_PID)).toBe(true);
        ...
        it("tags relay peer in peerStore on track", async () => {
            ...
            expect(helia.libp2p.peerStore.merge)
                .toHaveBeenCalledWith(remotePeer, {
                tags: {
    ...
    describe("dialRelay (via addExternalRelays)", () => {
        ...
        it("successful dial tracks relay", async () => {
            ...
            expect(rd.relayPeerIds.has(RELAY_PID))
                .toBe(true);
            expect(rd.relayEntries()).toContainEqual(expect.objectContaining({
                peerId: RELAY_PID,
            }));
            expect(upsertCachedRelay)
                .toHaveBeenCalledWith(RELAY_PID, RELAY_ADDRS);
            rd.stop();
        });
    ...
    describe("discoverRelays", () => {
        ...
        function makeProvider(pid, addrs) {
            return {
                id: { toString: () => pid },
        ...
        it("finds and dials DHT provider", async () => {
            ...
            expect(helia.libp2p.dial)
                .toHaveBeenCalled();
            expect(upsertCachedRelay)
                .toHaveBeenCalledWith(PROVIDER_PID, PROVIDER_ADDRS);
            rd.stop();
        });
...
//# sourceMappingURL=peer-discovery.test.js.map